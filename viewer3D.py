...
class Viewer3D(object):
    ...
    def draw_map(self, slam, map_file_path):
        ...
        file_name = os.path.splitext(os.path.basename(map_file_path))[0]

        # Save the map to the specified file path
        map_file_path = os.path.join("./pointcloud", file_name + ".csv") # or ".json"
        with open(map_file_path, 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['x', 'y', 'z', 'r', 'g', 'b'])
            for i, p in enumerate(map_state.points):
                writer.writerow([p[0], p[1], p[2], map_state.colors[i][0], map_state.colors[i][1], map_state.colors[i][2]])
